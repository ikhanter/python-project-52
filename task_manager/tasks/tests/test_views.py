...
class TestViews(TestCase):
    ...
    def setUp(self):
        ...
        self.tasks_update_url = reverse('tasks_update', args=[self.test_task.pk])
        self.tasks_delete_url = reverse('tasks_delete', args=[self.test_task.pk])
    ...
    def test_TasksCreateView_POST(self):
        # Unauthorized
        response = self.client.post(self.tasks_create_url, {
            'name': 'test_task2',
    ...
    def test_TasksUpdateView_POST(self):
        # Unauthorized
        response = self.client.post(self.tasks_update_url, {
        ...
        self.assertEquals(Task.objects.get(pk=self.test_task.pk).name, 'test_task_changed')
        self.assertEquals(Task.objects.get(pk=self.test_task.pk).description, 'test_description_changed')
        self.assertEquals(Task.objects.get(pk=self.test_task.pk).status.status_name, 'test_status2')
        self.assertEquals(Task.objects.get(pk=self.test_task.pk).executor.username, 'test_user1')
        self.assertEquals(response.status_code, 302)
